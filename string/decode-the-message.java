...
class Solution {
    public String decodeMessage(String key, String message) {
        ...
        for (int i = 0; i < key.length(); i++) {
            if (!key_modified.contains(key.substring(i, i + 1))) {
                key_modified += key.substring(i, i + 1);
            }
        }

        String answer = "";
        for(int i=0; i<message.length();i++){
            answer += message.charAt(i)!=' '?  alphabet.charAt(key_modified.indexOf(message.charAt(i))): ' ';
        }
